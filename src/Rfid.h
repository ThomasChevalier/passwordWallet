#ifndef RFID_HEADER_THOMAS_CHEVALIER
...
uint8_t rfid_pcd_read_register(uint8_t reg);
void rfid_pcd_read_register_multiple(uint8_t reg,uint8_t count,uint8_t *values,uint8_t rxAlign);
...
StatusCode rfid_pcd_transceive_data(uint8_t *sendData, uint8_t sendLen, uint8_t *backData, uint8_t *backLen, uint8_t *validBits /* = 0 */, uint8_t rxAlign /* = 0*/, uint8_t /*bool*/ checkCRC /* = false*/);
...
// Support functions
StatusCode rfid_pcd_MIFARE_transceive(uint8_t *sendData, uint8_t sendLen, uint8_t /* bool */  acceptTimeout /*= false*/);
