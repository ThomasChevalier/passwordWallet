...
{
	return fram_read_byte(PWD_ADDR(pwd_id, PWD_OFFSET_PREV_PWD_UTIL));
}

uint8_t password_read_next_pwd_util (uint8_t pwd_id)
{
	return fram_read_byte(PWD_ADDR(pwd_id, PWD_OFFSET_NEXT_PWD_UTIL));
}
...
{
	fram_write_byte(PWD_ADDR(pwd_id, PWD_OFFSET_PREV_PWD_UTIL), val);
}

void password_set_next_pwd_util (uint8_t pwd_id, uint8_t val)
{
	fram_write_byte(PWD_ADDR(pwd_id, PWD_OFFSET_NEXT_PWD_UTIL), val);
}
